...
import VideoMotionWorker from './motion.worker';
// type을 위해서 import
...
class VideoUtils implements MediaUtilsInterface {
    ...
    private tinyFaceDetectOption = new faceapi.TinyFaceDetectorOptions({
        inputSize: 160,
    });
    ...
    async _initialize() {
        ...
        try {
            ...
            throw new window.Entry.Utils.IncompatibleError('IncompatibleError', [
                window.Lang.Workspace.check_webcam_error,
            ]);
        }
        ...
        try {
            ...
            */
            this.motionWorker.onmessage = (e: { data: { type: String; message: any } }) => {
                ...
                if (window.Entry.engine.state !== 'run' && type !== 'init') {
                    return;
                }
                this.totalMotions = {
                    total: message.total,
            ...
            window.Entry.addEventListener('beforeStop', this.reset.bind(this));
            window.Entry.addEventListener('run', this.initialSetup.bind(this));
            ...
            if (this.isChrome) {
                ...
                this.worker.onmessage = (e: { data: { type: String; message: any } }) => {
                    ...
                    const modelLang = window.Lang.Blocks[`video_${name}_model`];

                    switch (type) {
                        case 'init':
                            if (message === 'warmup') {
                                window.Entry.toast.success(
                                    window.Lang.Msgs.video_model_load_success,
                                    window.Lang.Msgs.video_model_load_completed,
                                    true
                                );
                                window.Entry.dispatchEvent('hideLoadingScreen');
                                console.timeEnd('test');
                            } else {
                                window.Entry.toast.success(
                                    window.Lang.Msgs.video_model_load_success,
                                    `${modelLang} ${window.Lang.Msgs.video_model_load_success}`,
                                    false
                                );
                            }
                ...
                window.Entry.dispatchEvent('showVideoLoadingScreen');
            } else {
                ...
                Promise.all([
                    Promise.all([
                        ...
                        window.Entry.toast.success(
                            window.Lang.Msgs.video_model_load_success,
                            `${window.Lang.Blocks.video_face_model} ${window.Lang.Msgs.video_model_load_success}`,
                            false
                        );
                    }),
                    cocoSsd
                        ...
                        .then((cocoLoaded: any) => {
                            ...
                            window.Entry.toast.success(
                                ...
                                `${window.Lang.Blocks.video_object_model} ${window.Lang.Msgs.video_model_load_success}`,
                                false
                            );
                            console.timeEnd('test');
                        }),
                    posenet
                        ...
                        .then((mobileNetLoaded: any) => {
                            ...
                            window.Entry.toast.success(
                                window.Lang.Msgs.video_model_load_success,
                                // eslint-disable-next-line
                                `${window.Lang.Blocks.video_pose_model} ${window.Lang.Msgs.video_model_load_success}`,
                                false
                            );
                        }),
                ]).then(() => {
            ...
            this.stream = stream;
            this.canvasVideo = this.GEHelper.getVideoElement(video);
            this.video = video;
            this.stopVideo();
    ...
    initialSetup() {
        ...
        this.isRunning = true;
        this.GEHelper.drawDetectedGraphic();
    ...
    startDrawIndicators() {
        ...
        if (this.objects && this.indicatorStatus.object) {
            this.GEHelper.drawObjectBox(this.objects, this.flipStatus);
        }
        if (this.faces && this.indicatorStatus.face) {
            this.GEHelper.drawFaceEdges(this.faces, this.flipStatus);
        }
        if (this.poses && this.indicatorStatus.pose) {
            this.GEHelper.drawHumanPoints(this.poses.predictions, this.flipStatus);
            this.GEHelper.drawHumanSkeletons(this.poses.adjacents, this.flipStatus);
        }
        setTimeout(() => {
    ...
    turnOffWebcam() {
        this.GEHelper.turnOffWebcam(this.canvasVideo);
    }

    turnOnWebcam() {
        this.GEHelper.drawVideoElement(this.canvasVideo);
    }
    ...
    setOptions(target: String, value: number) {
        ...
        switch (target) {
            case 'transparency':
                this.GEHelper.setVideoAlpha(this.canvasVideo, value);
                break;
            case 'hflip':
                ...
                this.GEHelper.hFlipVideoElement(this.canvasVideo);
                break;
            case 'vflip':
                this.flipStatus.isChanged = true;
                this.flipStatus.vertical = !this.flipStatus.vertical;
                this.GEHelper.vFlipVideoElement(this.canvasVideo);
                break;
        }
    }
    ...
    reset() {
        ...
        if (this.canvasVideo) {
            this.GEHelper.setVideoAlpha(this.canvasVideo, 50);
        }
    ...
    async compatabilityChecker() {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            throw new window.Entry.Utils.IncompatibleError('IncompatibleError', [
                window.Lang.Workspace.check_browser_error_video,
            ]);
        }
